...
\\
\textit{Keywords}: synchrony, compensatory dynamics, environmental stochasticity, demographic stochasticity, interspecific competition, stability
\end{abstract}
...
\begin{equation}
\phi_{R,\mathcal{M}_{E}} = \frac{\sum_{i,j}p_ip_j\text{cov}(u_{ei},u_{ej})}{S^2}
\end{equation}
...
Across the five communities, our limiting case predictions closely matched synchrony from the corresponding simulation experiment (Table S1).
...
The largest difference between predicted and simulated synchrony was 0.05 in New Mexico for $\phi_{R,\mathcal{M}_D}$ (Table 3).
...
Results for synchrony of percent cover are qualitatively similar, but simulation results were more variable and less consistent with analytical predictions and observed synchrony (Figs. S2-S3).
...
We also found that analyses based on synchrony of species' percent cover were uninformative (Online Supporting Information), implying that it is difficult to decipher mechanisms of species synchrony from abundance time series, as expected by theory [@Loreau2008a].
...
However, that result appears to be an artifact. Removing environmental variation results in a negative invasion growth rate for *A. tripartita*. Although we only analyzed IBM runs in which *A. tripartita* had not yet gone extinct, it was at much lower abundance than in the other simulation runs.
...
*A. tripartita* has a facilitative effect on each grass species across all vital rates, except for a small competitive effect on *H. comata*'s survival probability (Tables S4, S9, S14).
At the same time, all the perennial grasses have negative effects on each other for each vital rate (Tables S4, S9, S14).
