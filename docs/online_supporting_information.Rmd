...
\begin{equation}
c_{ij} = \frac{B_{ij}}{1-a_i a_j} = \frac{N_{i}^* N_{j}^* \sigma_{ei} \sigma_{ej} \text{cov}(u_{ei},u_{ej}) + \sqrt{N_{i}^* N_{j}^*} \sigma_{di}\sigma_{dj} \text{cov}(u_{di},u_{dj})}{1 - (1-r_{mi})(1-r_{mj})}
\end{equation}

and the variance-covariance matrix of population growth rates becomes

\begin{equation}
R_{ij} = \frac{r_{mi}r_{mj}}{K_i K_j} C_{ij} + \sigma_{ei} \sigma_{ej} \text{cov}(u_{ei},u_{ej}) + \frac{\sigma_{di}\sigma_{dj} \text{cov}(u_{di},u_{dj})}{\sqrt{N_{i}^* N_{j}^*}},
\end{equation}
...
When all species have identical abundances and $p_i = 1/S$, where *S* is species richness, the both synchrony values equal 1/S [@Loreau2008a]. The prediction represented in Equation S20 is Equation 3 in the main text, which we refer to as $\phi_{R,\mathcal{M}_D}$.
...
\noindent The prediction represented in Equation S22 is Equation 4 in the main text, which we refer to as $\phi_{R,\mathcal{M}_E}$. If we know the covariance matrix of the species environmental responses, we can calculate the above expectations directly. Note that the environmental responses are normalized in the above equations, therefore the covariances are correlations in the above equations.
...
Supporting Tables
-----------------
```{r perc_diff_table, echo=FALSE, include=TRUE, results='asis', message=FALSE, eval=F, cache=FALSE, warning=FALSE}
library(reshape2)
library(plyr)
library(ggplot2)
library(xtable)
