...
When all species have identical abundances and $p_i = 1/S$, where *S* is species richness, the both synchrony values equal 1/S [@Loreau2008a]. The prediction represented in Equation S20 is Equation 3 in the main text, which we refer to as $\phi_{R,\mathcal{M_D}}$.
...
\noindent The prediction represented in Equation S22 is Equation 4 in the main text, which we refer to as $\phi_{R,\mathcal{M_E}}$. If we know the covariance matrix of the species environmental responses, we can calculate the above expectations directly. Note that the environmental responses are normalized in the above equations, therefore the covariances are correlations in the above equations.
