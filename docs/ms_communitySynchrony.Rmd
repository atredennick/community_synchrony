...
\begin{abstract}
Temporal asynchrony among species is an important mechanism through which diversity can stabilize ecosystem function, but identifying the mechanisms that determine synchrony remains a challenge.
...
Simulations experiments showed that the average synchrony of per capita growth rates, which can range from 0 (perfect asynchrony) to 1 (perfect synchrony), was higher when environmental variation was present (0.62) rather than absent (0.43).
...
Regression coefficients for the effect of crowding by each species can be considered a matrix of interaction coefficients whose diagonals represent intraspecific interations and whose off-diagonals represent interspecific interactions [@Adler2010]. These interaction coefficients can take positive (facilitative) or negative (competitive) values.
...
(3) IBM: Environmental stochastcity removed;
...
Only in Idaho did removing environmental fluctuations cause an increase in synchrony (Fig. 2), but the effect was small (9% increase; Table S1).
Overall, species' temporal random effects in the statistical vital rate models are positively correlated (Table S2).
Species interactions are weak in these communities [Table S3 and @Chu2015], and removing interspecific competition had little effect on synchrony (Fig. 2; "No Comp." simulations).
Removing competition caused, at most, a 5% change in synchrony (Table S2).
Removing demographic stochasticity ("No D.S." simulations) caused synchrony to increase slightly in all communities (Fig. 2), with an average 6% increase over synchrony from IBM simulations on a five meter area.
The desynchronizing effect of demographic stochasticity modestly counteracted the synchronizing force of the environment, but not enough to lower synchrony to the level observed when only demographic stochasticity is operating (Fig. 3).
...
The theoretical population dynamical model we used here also successfully predicted ecosystem stability in several biodiversity-ecosystem function experiments [@DeMazancourt2013a].
...
However, our results do contrast with a recent analysis of several biodiversity-ecosystem function experiments showing that competition drives species synchrony in grasslands [@Gross2014].
...
In theory, species interactions should weaken through time as community assembly progesses [@Kokkoris1999], meaning that in biodiversity-ecosystem function experiments the processes driving competitive exclusion are still operating.
