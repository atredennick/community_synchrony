...
Lastly, their analysis did not include data from grassland communities, which have received the most experimental [@Tilman2001; @Hector2010] and theoretical [@DeMazancourt2013a] attention.
Thus, we lack the ability to draw clear lines of inference from theoretical and experimental work to natural plant communities.
...
Simulation experiments revealed that removing environmental fluctuations has the largest impact on synchrony, leading to a reduction in synchrony of species growth rates in four out five communities (Fig. 2a).
...
Only in Idaho did removing environmental fluctuations cause an increase in synchrony (Fig. 2a), but the effect was small (9% increase).
Species interactions are weak in these communities [Table SX and @Chu2015], so removing interspecific competition had little effect on synchrony (Fig. 2a; "No Comp." simulations).
Removing competition caused, at most, a 5% change in synchrony (Table SX).
Removing demographic stochasticity ("No D.S." simulations) caused synchrony to increase slightly in all communities (Fig. 2a), with an average 6% increase.
...
DISCUSSION
----------
Our study produced three main findings that were generally consistent across five natural plant communities: (1) interspecific interactions were weak and therefore had little impact on community synchrony; (2) demographic stochasticity decreased community synchrony, as expected by theory, and its effect was largest in small populations; and (3) environmental fluctuations increased community synchrony relative to simulations in constant environments because species-specific responses to the environment were positively, but not completely, correlated.
...
Contrary to expectations, community synchrony was virtually indistinguishable between simulations with and without interspecific interactions (Fig. 2a).
...
Demographic stochasticity is able to partially counteract the synchronizing effects of environmental fluctuations and interspecific interactions on per capita growth rates, but only when populations are small (Fig. 2b), in agreement with theory [@Loreau2008a].
Even in small populations, however, demographic stochasticity is not a strong enough force to induce independent fluctuations (Fig. 2b).
...
Only in the Idaho community does synchrony of per capita growth rates approach independent fluctuations (1/*S*, where *S* is the number of specuies) in a non-constant environment (Fig. 2b).
...
First, of our two limiting-case theoretical predictions for species synchrony (Equations 3-4), only our prediction for communities driven by environmental fluctuations alone ($\mathcal{M}_E$) received empirical support (Fig. 1).
Second, in our model simulations, removing interspecific interactions resulted in no discernable change in community-wide species synchrony of per capita growth rates (Fig. 2a).
Lastly, removing environmental fluctuations from simulations consistently reduced synchrony (Fig. 2a), and the synchrony of species in monoculture is a very strong predictor of synchrony of species in polyculture (Fig. 3).
...
Only in the Idaho community did not removing environmental fluctuations not cause a large decrease in synchrony, but removing competition had no effect either.
...
The insensitivity of species synchrony to model perturbations in the Idaho community challenges the consistency of our results, but its outlier status may have more to do with environmental variability than intrinsic species synchrony.
Regardless of how synchronously or asynchronously species in a community respond to environmental fluctuations, if the environment does not fluctuate substantially then removing such fluctuations will not impact species synchrony.
Indeed, the Idaho community had the lowest variability in percent cover over the observation period (Table 2).
At the same time, correlations among species' temporal random effects for each vital rate in the Idaho community were similar to, or greater than, those in the other communities we studied (Table SX).
These lines of evidence lead us to conclude that either environmental fluctuations are weak in the Idaho community or our model was unable to detect temporal fluctuations in the data.
In other words, species' responses to environmental conditions are likely the main determinant of synchrony in the Idaho community, but we were unable to detect the effect of the environment due to model misspecification or a lack of a strong environmental signal in the data.
...
When we removed environmental variation, we found that synchrony decreased in four out of the five grassland communities we studied (Fig. 2a).
...
\begin{figure}[!ht]
  ...
  \caption{Community-wide species synchrony of per capita growth rates from model simulation experiments. (a) Synchrony of species' growth rates for each study area from simulation experiments with demographic stochasticity, environmental stochasticity, and interspecific competition present (``All Drivers''), demographic stochasticity removed (``No D.S.''), environmental stochasticity removed (``No E.S.''), interspecific competition removed (``No Comp.''), interspecific competition and demographic stochasticity removed (``No Comp. + No D.S.''), and interspecific competition and environmental stochasticity removed (``No Comp. + No E.S.''). Abbreviations within the bars for the Arizona site indicate whether the IBM or IPM was used for a particular simulation. (b) Synchrony of species' growth rates for each study area from IBM simulations across different landscape sizes when all drivers are present (``All Drivers'') and where only demographic stochasticity is present (``No E.S. + No Comp.''). The strength of demographic stochasticity decreases as landscape size increases.}
\end{figure}
