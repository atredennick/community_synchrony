...
\textcolor{red}{Indeed, results from an IBM with infinite population size would equal results from the IPM.
Given computational constraints, the largest landscape we simulate is a 5 square meter plot.}
...
All other IBM simulations were run on a 5 square meter landscape.
...
Across the five communities, our limiting case predictions closely matched synchrony from the corresponding simulation experiment (Table 3).
...
Only in Idaho did removing environmental fluctuations cause an increase in synchrony (Fig. 1), but the effect was small (9% increase; Table S1).
Overall, species' temporal random effects in the statistical vital rate models are positively correlated (Table S2).
Species interactions are weak in these communities [Table S3 and @Chu2015], and removing interspecific competition had little effect on synchrony (Fig. 1; "No Comp." simulations).
...
\textcolor{red}{The desynchronizing effect of demographic stochasticity, which increases as population size decreases, modestly counteracted the synchronizing force of the environment, but not enough to lower synchrony to the level observed when only demographic stochasticity is operating (Fig. 2).
Synchrony from simulations with demographic and environmental stochasticity decreased as the simulated area decreased, representing a shift from synchrony being driven by environmental responses (e.g., $\mathcal{M}_E$) to being driven by a combination of demographic and environmental stochasticity (e.g., between $\mathcal{M}_E$ and $\mathcal{M}_D$).}
...
Results for synchrony of percent cover are qualitatively similar (Figs. S2-S3) but more variable, as expected by theory [@Loreau2008a].
...
\textcolor{red}{Our analysis of how demographic stochasticity affects synchrony across population sizes also demonstrates that synchrony depends on the observation area.
...
The apparent inconsistency between our results and those of @Gross2014 disappears when we consider the differences between our studies.
...
In theory, species interactions should weaken through time as community assembly processes [@Kokkoris1999], meaning that in biodiversity-ecosystem functioning experiments the processes driving competitive exclusion are still operating.
...
\textcolor{blue}{A tempting conclusion is that our study confirms that compensatory dynamics are rare in natural communities, and that ecologically-similar species will exhibit synchronous dynamics [e.g., @Houlahan2007].
...
Such a conclusion is in contrast to a recent analysis of several biodiversity-ecosystem functioning experiments showing that competition drives community synchrony [@Gross2014].
