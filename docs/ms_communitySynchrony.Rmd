...
Experimental work in grassland communities provides strong evidence for the stabilizing effect of asynchronous population dynamics [@Hector2010 ;@DeMazancourt2013a; @Gross2014], but how theoretical mechanisms of species asynchrony link to observed dynamics remains unclear [@Loreau2008a; @Gonzalez2009].
Futhermore, analyses of observational time series tend to show synchronous, rather than asynchrounous, population dynamics among species of similar ecology [@Houlahan2007; @Mutshinda2009; @Rocha2011; @Vasseur2014].
...
The relative strength of those three forces determines if species fluctuate synchronously or not [@Loreau2008a].
The seminal study by @Houlahan2007 suggested that asynchronous population fluctuations are rare because they found generally positive correlations in species' abundances through time across a large number of ecosystems.
Based on that result, @Houlahan2007 concluded that abiotic factors synchronize species' abundances.
Such a conclusion rests on the implicit assumption that species will fluctuate independently in the absence of temporal niche differences and that strong interspecific interactions cause compensatory dynamics.
...
However, independent fluctations may not be the correct benchmark against which to compare observed dynamics, and interspecific competition actually synchronizes species dynamics [@Loreau2008a]. 
...
Based on the above theory, we aimed to test the following four hypotheses related the synchrony of per capita growth rates:

1. In large populations ($\sigma^{2}_{d} \approx 0$), removing interspecific interactions should lead to lower synchrony if interspeficic interactions ($\sigma^{2}_{c}$) are strong;
2. In small and medium-sized populations ($\sigma^{2}_{d} \neq 0$), removing interspecific interactions should have the same effect as hypothesized in (1), and removing environmental stochasticity should lead to lower synchrony if environmental forcing ($\sigma^{2}_{e}$) is strong;
3. In small and medium-sized populations ($\sigma^{2}_{d} \neq 0$), synchrony of per capita growth rates ($\phi_r$) should equal the inverse of the number of species in the community (1/*S*) when only demographic stochasticity is present ($\sigma^{2}_{c}$ and $\sigma^{2}_{e} \approx 0$);
...
Chu and Adler 2015 describe the selection of focal species in each data set, so we offer a brief explanation here.
...
The location (latitude and longitude), biome, number of years in the time series, and selected species for each site are shown in Table 1. 
...
In brief, the crowding experienced by a focal genet is a function of the size of and distance to all neighboring genets.
See @Adler2010 and @Chu2015 for a thorough description of how crowding is estimated.
