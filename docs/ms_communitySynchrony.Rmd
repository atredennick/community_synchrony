...
Synchrony is highest when species' environmental responses are perfectly correlated, and declines as environmental responses diverge [@Ives2002; @Loreau2008a; @Loreau2013].
Alternatively, if environmental forcing is weak, interspecific competition can lead to compensatory dynamics that desynchronize species dynamics [@Lehman2000].
Of course, real communities are subject to some combination of environmental stochasticity, interspecific competition, and demographic stochasticity that complicate predictions from special theoretical cases [@Loreau2013].

The fact that species synchrony emerges from complex interactions among species' environmental responses, species interactions, and demographic stochasticity makes it difficult to disentangle the drivers of synchrony in observational data [e.g., @Houlahan2007].
...
Therefore, inference on the processes that drive synchrony from metrics of synchrony, like summed community covariance, alone is prone to error.
@Mutshinda2009 attempted to overcome some of the limitations of inferring process from pattern by fitting a dynamic population model to several community time series of animal abundance.
...
However, Mutshinda et al.'s (2009) modeling technique relied on abundance data that may or may not reliably capture competitive interactions that occur at the individual level.
Thus, despite several efforts to determine the drivers of species synchrony in natural communities, our understanding remains limited.

Much like research on diversity-stability relationships, empirical work has lagged behind theoretical work aimed at deepening our mechanistic understanding of species synchrony.
...
First, we can use the models the directly compare synchrony from communities with and without different processes operating.
...
In combination, our dynamic modeling approach represents the most direct means toward unraveling the process that jointly determine community dynamics, species synchrony, and, ultimately, ecosystem stability.
...
\noindent All our code and the data necessary to reproduce the results reported in this paper can be found on GitHub (link) as release v1.0, and we have archived that release on Dryad (link).
...
Using the community time series of per capita growth rates in percent cover, we calculated community synchrony using the metric of @Loreau2008a in the `synchrony' package [@Gouhier2014] in R [@R2013].
...
\begin{equation}
\text{E}(\phi_r) = \frac{1}{\left(\sum_{i}p_{i}^{1/2}\right)^{2}}
\end{equation}

\noindent where $p_i$ is the average frequency of species *i* ($p_i = N_i/N_Total$).  
