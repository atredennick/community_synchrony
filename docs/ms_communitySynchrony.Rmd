...
Introduction
------------
The temporal synchrony of population dynamics among interacting species largely determines ecosystem stability [@Ives2002; @Loreau2013; @Gross2014] and is a function of demographic stochasticity, environmental stochasticity, and competition [@Loreau2008a; @Loreau2013].
Strong asynchrony (also called compensatory dynamics) can emerge from negative interactions among competitors [@Lehman2000], but can also be the result of species-specific responses to environmental conditions [@Loreau2013].
However, it may often be the case that communities are composed of species that respond similarly to environmental conditions, which can synchronize community dynamics [@Houlahan2007; @Rocha2011].
Competition, too, may synchronize dynamics under some conditions [@Loreau2013].
In fact, demographic stochasticity is the only force with a consistent desynchronizing effect on population dynamics [@Loreau2008a].
Since environmental forces and competition can both lead to strong synchronous or asynchronous dynamics, deciphering the mechanisms behind species synchrony in natural communities has been a challenge.

One of the largest hurdles to understanding community synchrony has been how we define synchrony.
Unfortunately, the null expectation of independent species fluctutations and the use of summed community covariance as a metric of synchrony resulted in the perception that synchrony and asynchrony are binary states [@Ives1995; @Klug2000; @Fischer2001; @Houlahan2007], when in fact synchrony is a continuous property [@Loreau2008a].
Fortunately, two recent theoretical advances clarify how we should view community syhnchrony.
First, there is no null expectation of community synchrony.
Theory identifies several limiting cases, but none can be considered null expectations [@Loreau2008a; @Loreau2013].
For example, independent fluctuations among species is one expected theoretical outcome, but only when demographic stochasticity alone is operating and all species have equal abundances and variances [@Loreau2008a].
Another expectation is that species fluctuate in perfect synchrony when competition and demographic stochasticity are weak and species' responses to the environment are perfectly correlated [@Loreau2008a].
It is impossible to *a priori* assign a theoretical expectation to a natural community, suggesting we should abandon the use of any null expectation.
Second, @Loreau2008a derived a new metric of synchrony ($\phi$) from a multi-species dynamic population model, which makes clear that synchrony and asynchrony are not binary states, contrary to their common usage in the literature.
...
Thus, our focus should not be on whether or not (a)synchronous dynamics occur, but instead on the degree of (a)synchrony within a community and what process increase or decrease sychrony [e.g., @Gross2014].

Since there is no null expectation for synchrony, no breakpoint at which dynamics can be considered asynchronous or synchronous, and values of synchrony are not unique to specific processes, it is impossible to infer the relative strength of different mechanisms driving synchrony by calculating summed covariances of population time series [e.g., @Houlahan2007].
To overcome the limitations of analyzing community covariance alone, @Mutshinda2009 used a statistical approach to decompose fluctuations in abundance into contributions from environmental stochasticity and species' interactions.
In agreement with Houlahan et al.'s (2007) conclusions, @Mutshinda2009 found that community dynamics were driven by positively correlated environmental responses among species.
...
In this paper, we explicitly test theory on all the drivers of species synchrony by parameterizing and simulating dynamic multi-species population models fit to long term demographic data from five semi-arid grasslands [@Chu2015].
We make no *a priori* assumptions about expected synchrony or the potential for drivers of population dynamics to increase or decrease synchrony.
Instead, we use the fitted population models to simulate communities with and without interspecific interactions, environmental forcing, and demographic stochasticity.
...
Asynchrony in population dynamics (also called compensatory dynamics) among interacting species is a key mechanism by which biodiversity can stabilize aggregate community or ecosystem function [@Ives2002; @Gonzalez2009; @Loreau2010; @Loreau2013].
Experimental work in grassland communities provides strong evidence for the stabilizing effect of asynchronous population dynamics [@Hector2010; @DeMazancourt2013a; @Gross2014; @Hautier2014], but identifying what drives species synchrony in natural communities remains a challenge [@Loreau2008a; @Gonzalez2009; @Loreau2013].
The challenge lies in partitioning community synchrony among three mechanistic drivers of population fluctuations: inter and intraspecific density-dependence, environmental stochasticity, and demographic stochastcity [@Loreau2008a].
Ideally, each mechanism would produce a unique signature on community dynamics and we could infer mechanism by comparing observed synchrony to some null expectation [e.g., @Houlahan2007].
Unfortunately, theoretical work suggests there is no null expectation of community synchrony and the drivers of synchrony interact in complex ways that preclude inference from observations alone [@Loreau2008a; @Loreau2013].

The null expectation for species synchrony has previously been defined as independent species fluctuations.
Such a null is convenient because it allows the use of summed community covariance to infer whether competition or environmental forces drive community dynamics: if summed covariance is negative, then competition is most important; if summed covariance is positive, then environmental forces are most important [@Houlahan2007].
However, independent species fluctuations is only expected under very specific conditions where demographic stochasticity alone is operating and all species have similar growth rates and density-dependence [@Loreau2008a; @Loreau2013].
...
Theory identifies several limiting cases [@Loreau2008a], but none can be considered an expectation in natural communities where the strengths of internal and external forces are unkown.
For example, independent species fluctuations are expected if only demographic stochasticity if operating and species all have the same abundances and variance [@Loreau2008a].
Furthermore, asymmetry in species' growth rates, competitive effects, and density-dependence can all interact, making it difficult to identify general trends between synchrony and its drivers.
For example, competition generally, but not always, has a positive effect on synchrony of per capita growth rates and a negative effect on synchrony of abundances [@Loreau2013].
Since the mechanisms behind community synchrony are context-dependent, the best way forward is to analyze the drivers of synchrony in natural communities and then relate observed synchrony back to limiting theoretical cases.
In so doing, we can discover common and rare theoretical cases in nature.
...
A major roadblock to understanding community synchrony is that many proposed mechanisms lead to similar outcomes.
Asynchronous fluctuations in species' abundances can result from interspecific competition [@Tilman2000] or species-specific responses to variable environmental conditions [@Loreau2013].
However, the effects of interspecific competition and environmental fluctuations are not as general as they appear.
For example, interspecific competition can have either a positive or negative effect on synchrony depending on how strongly species interact, the symmetry of those interactions, and species' growth rates [@Loreau2013].
Likewise, environmental fluctuations can have either a positive or negative effect on synchrony depending on how similarly species respond to environmental conditions.
Since predictions arising from the above mechanisms are not mutually exclusive [@Ranta2008], it is difficult to infer mechanism from population time series [e.g., @Houlahan2007].

Inferring the mechanism driving synchrony in observed population time series has also been hampered by the widespread use of summed community covariance as a metric for detecting compensatory dynamics [e.g., @Ives1995; @Klug2000; @Fischer2001; @Houlahan2007].
...
Aside from issues on how to measure synchrony, disentangling the factors that drive species synchrony is difficult.
...
@Mutshinda2009 used a statistical technique to decompose synchrony of species' abundances calculated from five time series.
