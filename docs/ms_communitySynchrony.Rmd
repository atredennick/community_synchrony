...
\textit{Authorship}: ATT, CdM, and PBA designed the research, ATT performed the research, and ATT, CdM, ML, and PBA contributed to the analyses and interpretation of results. All authors contributed to writing the manuscript.

\textit{Keywords}: synchrony, compensatory dynamics, environmental stochasticity, demographic stochasticity, stability

\textit{Running Title}: Species synchrony in grasslands

\textit{Article Type}: Letter

\textit{Number of Words}: Abstract = ; Main Text = 

\textit{Number of References}: 

\textit{Number of Tables and Figures}:  Tables;  Figures

\textit{Corresponding Author}: Andrew Tredennick \\  
...
\noindent where $\sigma_{r_{i}}$ is the temporal variance of species *i*'s per capita population growth rate ($r_i$) and $\sigma^{2}_{T}$ is the temporal variance of the aggregate community-level growth rate. 
...
Our results show that community-wide synchrony is predominantly driven by environmental forcing across five perennial plant communities.
Thus, our results support Loreau and de Mazancourt's (2008) prediction that community-wide synchrony of per capita growth rates should approximately equal the synchrony of species' environmental responses when demographic stochasticity and species interactions are weak.
We have reason to believe that our results are quite general since many studies report strong niche differences in plant communities; meaning that species interactions are weak.
