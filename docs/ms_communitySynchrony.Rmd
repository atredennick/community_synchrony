...
INTRODUCTION
------------
The relationship between species richness and ecosystem stability in plant communities depends on species synchrony [@Tilman2006; @Isbell2009; @Hector2010; @Hautier2014].
...
In the absence of other forces, interspecific competition generally desynchronizes population abundances but synchronizes per capita growth rates [@Loreau2008a; @Loreau2013; @Gross2014], except in the special case where competition is weak and population growth rates are high [@Loreau2013].
...
In the absence of environmental fluctuations and interspecific competition, and in communities composed of species with similar growth rates, abundances, and demographic variance, demographic stochasticity leads to independent species fluctuations and always desynchronizes population dynamics [@Loreau2008a; @DeMazancourt2013a].
...
In this paper, we refine previous theory that has been used to predict the effects of species richness on ecosystem stability [@DeMazancourt2013a] and species synchrony [@Loreau2008a].
...
We also use simulation results from species growing in monoculture to estimate the covariance of species' responses to the environment ($\text{cov}(\mu_{ie}, \mu_{je})$) for parameterizing Equations 5-6.
...
However, IBM-simulated synchrony is consistently lower than IPM-simulated synchrony (Fig. X1), likely due to the desynchronizing effect of demographic stochasticity.
...
Our prediction under $\mathcal{M}_E$ for percent cover did not fair as well, with a slope and intercept that were significantly different from zero or one, respectively.
The regression lines between observed and predicted growth rates under prediction $\mathcal{M}_D$ (demographic stochasticity only) had intercepts and/or slopes significantly different from zero or one, respectively, for both percent cover and per capita growth rates.
...
DISCUSSION
----------
Our study produced three main findings that were consistent across five natural plant communities: (1) interspecific interactions were weak and therefore had little impact on community synchrony; (2) demographic stochasticity decreased community synchrony, as expected by theory, and its effect was largest in small populations; and (3) environmental fluctuations increased community synchrony relative to simulations in constant environments because species-specific responses to the environment were positively, but not completely, correlated.
These results are in agreement with our theoretical prediction for species synchrony under the limiting case of community dynamics being driven by environmental fluctuations alone.
For all of our results, from empirical tests of theoretical predictions to our model simulations, synchrony of per capita growth rates were more consitent and predictive of community dynamics that percent cover.
Thus, we primarily focus on those results in our discussion, and echo the call of @Loreau2008a to view species synchrony through the lens of per capita growth rates.
...
Our theoretical prediction for species synchrony in a neutral community with no species interactions or environmental fluctuations ($\mathcal{M}_D$) received no support when parameterized with emprical estimates (Fig. 1).
...
In large populations where interspecific interactions are weak, synchrony is expected to be driven exclusively by environmental fluctuations (Equations 5-6).
...
Several lines of evidence lead us to conclude that environmental fluctuations drive species synchrony in our focal plant communities.
First, of our two limiting-case theoretical predictions for species synchrony (Equations 3-6), only our prediction for communities driven by environmental fluctuations alone ($\mathcal{M}_E$) received empirical support (Fig. 1).
Second, in our model simulations, removing interspecific interactions resulted in no discernable change in community-wide species synchrony of per capita growth rates (Fig. 2).
Lastly, removing environmental fluctuations from simulations consistently reduced synchrony (Fig. 3), and the synchrony of species in monoculture is a very strong predictor of synchrony of species in polyculture (Fig. 4).
If we consider synchrony in monoculture to represent the synchrony of species' responses to the environment, then our results lead to only one conclusion: environmental fluctuations are the primary driver of community-wide species synchrony.
...
In fact, when we ran simulations with the shrub removed, synchrony of the Idaho community increased substantially and was similar to synchrony in the Kansas community (Fig. SX), which also contained three species.
...
We found strong support for our theoretical prediction that community-wide synchrony of per capita growth rates should approximately equal the synchrony of species' environmental responses when demographic stochasticity and species interactions are weak: removing environmental fluctuations was the only simulation that affected synchrony and our limiting-case prediction where only environmental stochasticity is operating faired well when confronted with emprical estiamtes.
...
\begin{figure}[!ht]
  \centering
      \includegraphics[width=6in]{./components/ipm_pgr_plot.png}
...
\begin{figure}[!ht]
  \centering
      \includegraphics[width=6in]{./components/ibm_sims_fig.png}
