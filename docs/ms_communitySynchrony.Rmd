...
Across the five communities, our limiting case predictions closely matched synchrony from simulations designed to test each prediction (Table 3).
...
Our theoretical predictions were derived from a relatively simple model of population dynamics and required several simplifying assumptions.
Yet, our theoretical predictions were strongly supported by our empirically-based model simulations of natural plant communities (Table 3).
...
Thus, there is accumulating evidence, now from experimental and observational studies, that the population dynamical approach is an extremely useful modeling framework for linking population dynamics to community and ecosystem level properties [@Loreau2010].
...
Even in small populations, however, demographic stochasticity is not a strong enough to completely counteract the synchronizing effects of environmental fluctuations and induce independent species fluctuations (Fig. 3).
...
However, Idaho's outlier status is soley due to its functional diversity.
When we removed the shrub (*Artemisia tripartita*) from our analyses, results from the Idaho data set conformed with results from all other sites: removing environmental stochasticity caused a significant reduction in species synchrony (Fig. SX).
