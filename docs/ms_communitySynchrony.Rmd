...
\normalsize{Andrew T. Tredennick\footnote{Correspondance: atredenn@gmail.com}\textsuperscript{1}, Claire de Mazancourt\textsuperscript{2}, Michel Loreau\textsuperscript{2}, and Peter B. Adler\textsuperscript{1}}
...
*Authorship*: ATT, CdM, and PBA designed the research, ATT performed the research, and ATT, CdM, ML, and PBA contributed to the analyses and interpretation of results. All authors contributed to writing the manuscript.
...
As a result, environmental variation caused an increase in synchrony relative to an expectation of independent species' responses, but synchrony was not perfect.
Species interactions tend to be weak, so removing interspecific competition did not affect synchrony. 
...
Introduction
------------
Biodiversity tends to stabilize ecosystem function in fluctuating environments [@Hector2010; @DeMazancourt2013a; @Hautier2014].
In the absence of a highly dominant and stable species, asynchrony in population dynamics (also called compensatory dynamics) among interacting species is the only way for biodiversity to stabilize aggregate community or ecosystem function [@Ives2002; @Loreau2010; @Loreau2013].
Experimental work in grassland communities provides strong evidence for the stabilizing effect of asynchronous population dynamics [@Hector2010; @DeMazancourt2013a; @Gross2014; @Hautier2014], but how theoretical mechanisms of species asynchrony link to observed dynamics remains unclear [@Loreau2008a; @Gonzalez2009].
Futhermore, analyses of observational time series tend to report synchronous, rather than asynchrounous, population dynamics among species of similar ecology [@Houlahan2007; @Mutshinda2009; @Rocha2011; @Vasseur2014].
...
One view is that density-dependent processes like interspecific competition lead to compensatory dyanmics [e.g., @Elton1942; @Tilman1988; @Ives1995].
...
Clearly, these forces are not mutually exclusive, and most research has focused on determining the relative strength of biotic and abiotic forcing on the synchrony of community dynamics [e.g., @Ives1995; @Klug2000; @Houlahan2007; @Mutshinda2009].
However, recent theory [@Loreau2008a] suggests that previous interpretations of time series have been impaired by a (1) faulty metric of synchrony that treated synchrony and asynchrony as binary states, (2) biased null expectations of species synchrony, and (3) the use of population abundances that are strongly influenced by lag effects to estimate immediate responses to the environment and competition.
Moreover, interspecific competition may actually synchronize, rather than de-synchronize, population dynamics [@Loreau2008a; @Loreau2010].
...
Unfortunately, reliance on such a metric created the search for binary states -- synchronous or not, compensatory dynamics or not -- where a continuum exists [@Loreau2008a].
Likewise, values of summed covariance are not exclusive to particular combinations of drivers, meaning that negative summed covariances can ocurr in the absence of competition [@Ranta2008].
...
In the end, the fact that few communities exhibit negative summed covariance [e.g., @Houlahan2007] yields little insight as to whether communities tend to be sychronized by environmental forces or not [@Loreau2010].

To overcome these limitations, @Loreau2008a derived a new metric of species synchrony based on a multi-species population dynamic model.
Their proposed metric, best suited to measure community-wide synchrony of per capita population growth rates, is

\begin{equation}
\phi_r = \frac{\sigma^{2}_{T}}{(\sum_{i}\sigma_{r_{i}})^{2}}
\end{equation}

where $\sigma_{r_{i}}$ is the temporal variance of species *i*'s per capita population growth rate ($r_i$) and $\sigma^{2}_{T}$ is the temporal variance of the aggregate community-level growth rate. 
...
Rather, the synchrony of community dynamics lies along a continuum from perfect asynchrony ($\phi_r=0$) to perfect synchrony ($\phi_r=1$).
...
For example, the seminal study by @Houlahan2007 suggested that asynchronous population fluctuations are rare because they found generally positive correlations in species' abundances through time across a large number of ecosystems.
...
In constrast to virtually every study on synchrony that uses population abundance (CITATIONS), @Loreau2008a suggest community-wide synchrony is best viewed through the lense of per capita growth rates.
...
On practical grounds, positively covarying abundances and negatively covarying environmental responses are not mutually exclusive [@Loreau2008a].
...
However, the @Mutshinda2009 study did not include data from plant communities that have, so far, received the most evidence for a strong role for interspecific interactions in driving species asynchrony [@Gross2014].
...
Results
-------
### Observed and simulated community synchrony
Synchrony of species' per capita growth rates at our study sites ranged from 0.36 to 0.89 (Table 2).
Synchrony tended to be higher in communities with few species (Arizona and New Mexico).
...
All sites exhibited more synchronous dynamics than expected if only demographic stochasticity was operating (E($\phi_r$) = 1/*S* in Table 2).
The difference between observed $\phi_r$ and E($\phi_r$) represents the effect of the environment and species interactions combined .
The combined effect of the environment and species interactions is positive in all communities (E($\phi_c + \phi_e$) in Table 2), indicating a synchronizing effect of those forces that counter-balance the desynchronizing effect of demographic stochasticity.
For all five communities, species synchrony from IPM and IBM simulations closely approximated observed synchrony (Fig. 1).

### Effect of removing interspecific interactions
The statistical models for vital rates showed that interspecific interactions were weak in all communities (Table X1), as previously found by @Chu2015.
Consistent with weak interspecific interactions, removing such interactions from IPM simulations resulted in no discernable changes in species synchrony of per capita growth rates or percent cover (Fig. 2).
This indicates that community regulation of population dynamics via coupled density-dependence ($\sigma_c$) is relatively weak in these communities.

The effect of removing interspecific interactions on synchrony in IBM simulations was also small for per capita growth rates and was not affected by population size (Fig. 3, top panels).
...
TODO: CHECK MEAN COVER VERSUS LANDSCAPE SIZE FOR EACH COMMUNITY
...
Can we generalize our results to a vast majority of plant communities?

A recent analysis of several biodiversity-ecosystem function experiments suggests our results are anomolous.
...
In fact, they found very weak correlations among species growin in polyculture and species growing in monoculture.
However, their results are based on experimentally-assembled communities that have coexisted for, at most, 11 years.
Thus, community assembly processes that can result in competitive exclusion may yet to have been realized and transient strong interspecific competition could be masking the synchronizing effects of the environment.
In contrast, the communities we studied consist of species that have coexisted for decades and have virtually zero niche overlap [@Chu2015].
...
```{r get_all_synch, echo=FALSE, include=TRUE, eval=T}
...
```{r synchrony_table, echo=FALSE, include=TRUE, results='asis', message=FALSE, eval=T}
