...
INTRODUCTION
------------
The earth's ecosystems are in a period of upheaval due to species extinctions [@Cardinale2012], rapid changes in community composition [@Dornelas2014a; @Vellend2013], and human-driven changes in the abiotic environment [@Vitousek1997].
...
The mechanisms that underpin species synchrony are complex [@Loreau2008a], and whether synchrony in grassland communities is primarily an outcome of species-specific responses to environmental conditions [@Hautier2014] or competition [@Gross2014] remains controversial [reviewed in @Gonzalez2009].
Fortunately, theoretical studies have identified the three main determinants of species synchrony as environmental stochasticity, demographic stochasticity, and interspecific competition [@Loreau2008a; @Gonzalez2009; @Loreau2013].
Unfortunately, quantifying the relative strengths of each driver on the degree of synchrony from community time series [e.g., @Houlahan2007] is impossible because an unbiased null expectation for synchrony does not exist [@Loreau2008a] and observed synchrony can arise from non-unique combinations of factors [@Ranta2008].
...
Furthermore, their analysis did not include data from grassland communities, which have received the most experimental [@Tilman2001] and theoretical [@DeMazancourt2013a] attention.
Thus, there remains a gap in our ability to draw clear lines of inference from theoretical and experimental work to natural plant communities.
...
First, we refine previous theory that has been used to predict the effects of species richness on ecosystem stability [@DeMazancourt2013a] and species synchrony [@Loreau2008a].
...
Our objective is to derive analytical predictions for species synchrony under special limiting cases.
We constrain our focus to synchrony of per capita growth rates, rather than abundance, because growth rates represent the instantaneous response of species to the environment and competition, and are less susceptible to the legacy effects of drift and disturbance [@Loreau2008].
To derive analytical predictions we...CLAIRE'S TEXT ON DERIVING THE PREDICTION...

### Results
We derived predictions for synchrony of species' growth rates for two limiting cases that are analytically tractable.
...
These simulations allowed us to isolate the effect of removing competition in large populations where demographic stochasticity is absent.
We also used simulation results from species growing in monoculture to estimate the covariance of species' responses to the environment ($\text{cov}(\mu_{ie}, \mu_{je})$) for parameterizing Equation 3.
...
The regression lines between observed and predicted growth rates under prediction $\mathcal{M}_D$ (demographic stochasticity only) had intercepts and/or slopes significantly different from zero or one, respectively (Fig. 1).

Simulation experiments reveal that removing environmental fluctuations has the largest impact on synchrony, leading to a reduction in snychrony of species growth rates in four out five communities (Fig. 2a).
Removing environmental fluctuations ("No E.S" simulations) decreased synchrony by X% in Arizona, X% in Kansas, X% in Montana, and X% in New Mexico.
Only in Idaho did removing environmental fluctuations cause an increase in synchrony (Fig. 2a), but the effect was small (X% increase).
Species interactions are weak in this communitie (Table SX), so removing interspecific competition had little effect on synchrony (Fig. 2; "No Comp." simulations).
Removing demographic stochasticity ("No D.S." simulations) caused synchrony to increase slightly in all communities (Fig. 2a).
The desynchronizing effect of demographic stochasticity was able to modestly counteract the synchronizing force of the environment, but not enough to match the synchrony of species when only demographic stochasticity is operating (Fig. 2b).
...
Results for synchrony of percent cover are qualitatively similar (Figs. SX-SX) but more noisy, as expected by theory [@Loreau2008].
...
\begin{figure}[!ht]
  ...
  \caption{Community-wide species synchrony of per capita growth rates from model simulation experiments.}
\end{figure}
