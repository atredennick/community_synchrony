...
\textbf{Number of Tables and Figures}:  2 Tables;  3 Figures
...
\begin{abstract}
Temporal asynchrony among species \hl{helps} diversity \hl{to} stabilize ecosystem functioning, but identifying the mechanisms that determine synchrony remains a challenge.
Here, we refine and test theory showing that synchrony depends on three factors: species responses to environmental variation, interspecific \hl{interactions}, and demographic stochasticity. We then conduct simulation experiments with empirical population models to quantify the relative importance of these factors in five plant communities.
\hl{We found} that the average synchrony of per capita growth rates, which can range from 0 (perfect asynchrony) to 1 (perfect synchrony), was higher when environmental variation was present (0.62) rather than absent (0.43).
Removing interspecific \hl{interactions} and demographic stochasticity had small effects on synchrony. In these plant communities, where species interactions and demographic stochasticity have little influence, synchrony reflects the covariance in species responses to the environment.
\end{abstract}
...
The stabilizing effect of species richness stems from individual species responding \hl{in different ways} to environmental fluctuations (environmental stochasticity), or fluctuating asynchronously because of random chance events (demographic stochasticity) [@Hector2010; @Isbell2009; @DeMazancourt2013a].
...
\hl{The link between synchrony and stability means that a mechanistic understanding of synchrony can help us predict the impacts of global change on ecosystem stability.}

Theory identifies three main determinants of species synchrony: environmental stochasticity, demographic stochasticity, and \hl{interspecific interactions} [@Loreau2008a; @Gonzalez2009; @Loreau2013].
For example, in a community composed of large \hl{populations} (no demographic stochasticity) \hl{with weak interspecific interations}, community-wide species synchrony should be determined by the covariance of species' responses to the environment [@Loreau2008a].
However, \hl{this} prediction relies on a relatively simple population model and only holds under two assumptions: (i) species' responses to the environment are similar in magnitude and (ii) all species have similar growth rates.
\hl{Whether such theoretical predictions hold in natural communities where species differences are unlikely to be symmetrical is unkown because few studies have explicitly tested theory on the drivers of species synchrony in natural communities} [@Mutshinda2009; @Thibaut2012], \hl{and they did not consider demographic stochasiticity.}
...
In part, controversy remains because quantifying the relative strengths of each driver on the degree of synchrony from \hl{the covariance matrix of species abundances} [e.g., @Houlahan2007] is impossible.
...
The best way to quantify the effects of environmental stochasticity, demographic stochasticity, and interspecific \hl{interactions} is to remove them one-by-one, and in combination.
...
@Thibaut2012 \hl{used a similar approach for reef fish and came to a similar conclusion: environmental responses determine synchrony.}
...
Furthermore, although both studies quantified the relative importance of environmental stochasticity and \hl{interspecific interactions} to explain the observed variation of species synchrony, they did not use the model to quantify how much synchrony would change when each factor is removed.
...
Our objectives are to (1) derive and test theoretical predictions of species synchrony and (2) determine the relative influence of environmental stochasticity, demographic stochasticity, and interspecific \hl{interactions} on species synchrony in natural plant communities.
...
Second, we use the multi-species population models to perform simulation experiments that isolate the effects of environmental stochasticity, demographic stochasticity, and interspecific \hl{interactions} on community-wide species synchrony.
...
\hl{The limiting case predictions we derive serve as baselines to help us interpret results from empirically-based simulations (described below).}
...
We present equivalent results for synchrony of species abundances in the Online Supporting Information, and show that they lead to the same overall conclusions as synchrony of per capita growth rates.
...
To derive analytical predictions we solved a first-order approximation of Equation 2 [@DeMazancourt2013a and Online Supporting Information].
\hl{Due to the linear approximation approach, our analytical predictions will likely fail in communities where species exhibit large fluctuations due to limit cycles and chaos} [@Loreau2008a].
\hl{Indeed, one of the advantages of focusing on growth rates rather than abundances is that growth rates are more likely to be well-regulated around an equilibrium value, if the long-term average of a species' growth rate is relatively small} (e.g., $r < 2$).
...
Confronting our theoretical predictions with data requires estimates of species dynamics of large populations (no demographic stochasticity) in \hl{growing in isolation} (no interspecific interactions) to calculate the covariance of species' environmental responses.
...
We followed the approach of @Chu2015 to fit statistical models for survival, growth, and recruitment (see Online Supporting Information for full details).
...
\hl{Crowding was treated as a fixed effect without a temporal component because the 95\% credible intervals for random year effects on crowding broadly overlapped zero.}
Spatial (quadrat groupings) variation was treated as a random effect on the intercept and temporal (interannual) variation was treated as random effects on the intercept and the effect of genet size in the previous year (Online Supporting Information).
...
We performed simulation experiments where drivers (environmental stochasticity, demographic stochasticity, or interspecific \hl{interactions}) were removed one-by-one and in combination.
...
This retains intraspecific interactions, and thus density-dependence, and results in simulations where species are \hl{growing in isolation}.
\hl{We cannot definitively rule out the effects of species interactions on all parameters, meaning that a true monoculture could behave differently than our simulations of a population growing without interspecific competitors.}
...
(1) IBM: All drivers (environmental stochasticity, demographic stochasticity, or \hl{interspecific interactions}) present;
(2) IPM: Demographic stochasticity removed;
(3) IBM: Environmental stochasticity removed;
(4) IBM: \hl{Interspecific interactions} removed;
(5) IPM: \hl{Interspecific interactions} and demographic stochasticity removed;
(6) IBM: \hl{Interspecific interactions} and environmental stochasticity removed.
...
First, in the absence of \hl{interspecific interactions} and demographic stochasticity, \hl{populations can only fluctuate in response to the environment}.
...
\hl{Synchrony of per capita growth rates and CV of percent cover are positively correlated} (Pearson’s $\rho$ = 0.72).
...
Species interactions are weak in these communities [Table S4 and @Chu2015], and removing \hl{interspecific interactions} had little effect on synchrony (Fig. 1; "No Comp." simulations).
Removing \hl{interspecific interactions} caused, at most, a 5% change in synchrony (Fig. 1).
...
Results for synchrony of percent cover are qualitatively similar, but simulation results were more variable and less consistent with analytical predictions and observed synchrony (Online Supporting Information, Figs. S2-S3).
...
\hl{Observed synchrony of per capita growth rates was positively correlated with the variability of percent cover across our focal communities, which confirms that we are investigating an important process underlying ecosystem stability.}
...
\hl{For example, the species in our communities do not have equivalent environmental and demographic variances (Figs. S4-S7), as required by our predictions.}
However, the theoretical predictions \hl{closely matched results from} simulations of population models fit to long-term data from natural plant communities (Table 3).
Such \hl{strong agreement} between our analytical predictions and the simulation results should inspire confidence in the ability of simple models to inform our understanding of species synchrony \hl{even in complex natural communities, and allows us to place our simulation results within the context of contemporary theory}.
...
Even in small populations, however, demographic stochasticity was not strong enough to \hl{compensate} the synchronizing effects of environmental fluctuations and match the analytical prediction where only demographic stochasticity is operating ($\mathcal{M}_D$ in Fig. 2).
...
This is most likely due to the strong effect of demographic stochasticity on the shrub *Artemisia tripartita* since even a \hl{2}5 $\text{m}^2$ quadrat would only contain a few individuals of that species.
...
\hl{Most communities are dominated by few common species and many rare species}, \hl{meaning that the low number of common species in our focal commiunities is not unusual.} 
\hl{Rather, the generality of our results hinges upon the influence of rare species.
...
However, recent theory predicts that persistent rare species may actually experience even weaker interspecific competition, and thus stronger niche differences, than common species} [@Yenni2012], \hl{in which case synchrony would remain predominantly driven by environmental responses.}
...
The lack of an effect of \hl{interspecific interactions} on synchrony is in contrast to a large body of theoretical work that predicts a strong role for competition in creating compensatory dynamics [@Tilman1988] and a recent empirical analysis [@Gross2014].
...
\hl{Our conclusion that species interactions have little impact on synchrony only applies to single trophic level communities.
Species interactions almost certainly play a strong role in multi-trophic communities where factors such as resource overlap} [@Vasseur2007], \hl{dispersal} [@Gouhier2010], \hl{and the strength of top-down control} [@Bauer2014] \hl{are all likely to affect community synchrony.}
